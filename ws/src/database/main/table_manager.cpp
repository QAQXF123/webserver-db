...
namespace bustub{
	TableManager::TableManager(){
		...
		printf("src%s ======\n", src);
		//user:data/a:data/b:3:usernum:2:0:username:3:20:pwd:3:20:0
	...
	bool TableManager::GetValue(const std::string& tableName, uint64_t key, const std::string &colName, std::string &value){
		Tuple tuple;
		auto tableSchema = getTable_[tableName]->GetTableSchema();
		bool getTuple = GetTuple(tableName, key, &tuple);
		if(!getTuple) return false;
		int colIdx = tableSchema->GetColIdx(colName);
		if(colIdx == -1) return false;
		value = tuple.GetValue(tableSchema, colIdx).ToString();
		return true;
	...
	void TableManager::test1(){
		...
		for(int i = 0; i < n; i++){
			...
			Schema *schema = userTable->GetTableSchema(); 
