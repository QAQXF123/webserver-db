...
#include "../../a-include/storage/page/b_plus_tree_leaf_page.h"
...
INDEX_TEMPLATE_ARGUMENTS
page_id_t B_PLUS_TREE_LEAF_PAGE_TYPE::GetPrevPageId() const { 
...
int B_PLUS_TREE_LEAF_PAGE_TYPE::Insert(const KeyType &key, const ValueType &value, const KeyComparator &comparator) {
  ...
  int cmp = comparator(KeyAt(index), key);
...
void B_PLUS_TREE_LEAF_PAGE_TYPE::MoveHalfTo(BPlusTreeLeafPage *recipient) {
  int size = GetSize(), move_size = size / 2;
...
bool B_PLUS_TREE_LEAF_PAGE_TYPE::Lookup(const KeyType &key, ValueType *value, const KeyComparator &comparator) const {
  ...
  int index = KeyIndex(key, comparator);
  printf("look up of leaf index :%d\n", index);
  int cmp = comparator(key, KeyAt(index));
  //printf("comparator(key, KeyAt(index) : %d\n", cmp);
 // printf("key:%d -- KeyAt(index):%d", key, KeyAt(index));
  if ( cmp == 0) {
    *value = ValueAt(index);
   // printf("look up end\n");
    return true;
  }else{
    printf("not find in look up\n");
    //printf("key:%ld -- KeyAt(index):%d", key, KeyAt(index));
  }
  return false;
}
INDEX_TEMPLATE_ARGUMENTS
